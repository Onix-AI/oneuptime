 ...
 *
 * PATCHED by Onix: Create a new McpServer per session instead of reusing
 * the singleton. MCP SDK >=1.26.0 throws "Already connected to a transport"
 * if you call .connect() on the same McpServer twice.
 */
...
function createMCPHandler(tools: McpToolInfo[]): McpHandlerFunction {
  return async (
    ...
    try {
      ...
      await handleNewSession(req, res, apiKey || "", tools);
    } catch (error) {
      next(error);
    }
  };
 ...
 *
 * PATCHED: Each session gets its own McpServer instance because
 * McpServer.connect() can only bind to one transport at a time.
 * The upstream code uses a singleton which breaks on the second session.
 */
async function handleNewSession(
  ...
  // Register tool handlers on this per-session server
  registerToolHandlers(mcpServer, tools);
